...
export default function Hero({ lang, hero }) {
  const isEs = lang === 'es'
  // Unified CMS: if portfolio.json hero exists, use it (with innerHTML for <strong>), else fallback to hardcoded spec
  const titleA = hero?.h1a || 'Senior Full-Stack Engineer'
  const titleB = hero?.h1b || 'Real-Time 3D & AI Data Platforms'
  const badge = hero?.badge || (isEs ? 'Disponible remoto — GMT-5 · overlap EU/US' : 'Available for remote — GMT-5 · EU/US overlap')
  const subtitleHtml = hero?.subtitle || null // contains <strong> tags — render via innerHTML
  const stats = hero?.stats?.length ? hero.stats : null // use CMS stats if present, else METRICS fallback
  return (
      ...
      <div className="relative max-w-[1280px] mx-auto px-6 pt-16 pb-12 md:pt-24 md:pb-20">
        ...
        {subtitleHtml ? (
          <motion.p
            ...
            className="mt-5 text-[16px] md:text-[18px] leading-relaxed text-zinc-400 max-w-2xl [&_strong]:text-zinc-200 [&_strong]:font-semibold"
        ...
        {/* Metrics — CMS stats if present, else hardcoded METRICS */}
        <motion.div
          initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.26 }}
          className="mt-8 flex flex-wrap gap-3"
        >
          {(stats ? stats.map(s => ({ value: s.number, label: s.label, icon: TrendingDown })) : METRICS).map(m => (
        ...
        >
          ...
          <a href="https://github.com/miguelxlerion" target="_blank" rel="noreferrer"
