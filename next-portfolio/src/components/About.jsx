...
export default function About({ lang, about }) {
  const isEs = lang === 'es'
  // Unified: use CMS about if present, else Vite hardcoded spec
  const title = about?.title ? about.title.replace(/<[^>]*>/g, '') : null
  const paragraphs = about?.paragraphs?.length ? about.paragraphs : null
  const groups = about?.skills?.length ? about.skills.map(s => ({ title: s.title, items: s.tags })) : null
  return (
    <section id="about" className="max-w-[1280px] mx-auto px-6 py-16 md:py-24 border-t border-[#1a1a25]">
      ...
      >
        ...
        <h2 className="text-[28px] md:text-[36px] font-bold tracking-tight text-white leading-tight">
          {title || (isEs ? 'Bilingüe en Diseño & Ingeniería' : 'Bilingual in Design & Engineering')}
        </h2>
        {paragraphs ? (
          <div className="mt-4 space-y-3 max-w-3xl">
            {paragraphs.map((p, i) => (
              <p key={i} className="text-[15px] leading-relaxed text-zinc-400 [&_strong]:text-zinc-200" dangerouslySetInnerHTML={{ __html: p }} />
            ))}
          </div>
        ) : (
