...
const PROJECTS = [
  {
    ...
    links: [
      ...
      { label: 'Source Code', href: 'https://github.com/miguelxlerion/smartfactory-3d', icon: GitBranch },
    ],
  ...
  },
  {
    ...
    links: [
      { label: 'Source Code', href: 'https://github.com/miguelxlerion/Rag_X', icon: GitBranch, primary: true },
    ],
    mediaType: 'diagram',
  ...
  },
  {
    ...
    links: [
      { label: 'Source Code', href: 'https://github.com/miguelxlerion/Ingest_Pipeline', icon: GitBranch, primary: true },
    ],
    mediaType: 'none',
  },
]
...
export default function Projects({ lang, projects }) {
  const isEs = lang === 'es'
  // Unified: if portfolio.json projects exists, map it; else fallback to hardcoded spec (preserved)
  const list = projects?.length ? projects.map((pr, i) => {
    const isFirst = i === 0
    return {
      n: String(i + 1).padStart(2, '0'),
      icon: [Box, Database, Radio][i % 3],
      title: pr.title || 'Untitled',
      tags: pr.tech || pr.tags || [],
      problem: (pr.desc || '').split('Solution:')[0]?.replace('<strong>Problem:</strong>', '').trim() || pr.desc || '',
      solution: (pr.desc || '').split('Solution:')[1]?.trim() || pr.metric || '',
      // keep TODO placeholders from spec when CMS has no media
      mediaType: isFirst ? 'video' : i === 1 ? 'diagram' : 'none',
      mediaPlaceholder: isFirst ? '[INSERTAR GIF/VIDEO: Demostración de órbita 3D y overlay de telemetría]' : '[INSERTAR DIAGRAMA DE ARQUITECTURA: PDF → Celery/Redis → pgvector → LangChain → LLM]',
      descHtml: pr.desc,
      links: (pr.links || []).map(l => ({ label: l.label, href: l.href, icon: GitBranch, primary: i === 0 })),
      image: pr.image,
    }
  }) : null
  const display = list || PROJECTS
  return (
        ...
        {display.map((p, idx) => (
          ...
          >
            <div className="p-6 md:p-8">
              ...
              {p.descHtml ? (
                <p className="mt-3 text-sm leading-relaxed text-zinc-400 [&_strong]:text-zinc-200" dangerouslySetInnerHTML={{ __html: p.descHtml }} />
              ) : (
              ...
              {p.mediaType !== 'none' && (
                <div className="mt-4 rounded-xl border border-dashed border-[#2a2a3a] bg-[#0a0a0f] p-4">
                  {/* TODO: Reemplazar con GIF/Diagrama real */}
                    {p.image ? (
                      <img src={p.image} alt={p.title} loading="lazy" className="w-full aspect-video object-cover rounded-lg border border-[#1a1a25]" />
                    ) : p.mediaType === 'video' ? (
                      <div className="aspect-video rounded-lg bg-[#16161f] border border-[#1a1a25] grid place-items-center text-center p-6">
                        ...
                        <p className="text-xs font-mono text-zinc-500 max-w-md">{p.mediaPlaceholder}</p>
                      </div>
                    ) : (
                      <div className="rounded-lg bg-[#16161f] border border-[#1a1a25] p-6 grid place-items-center text-center">
                        ...
                        <p className="text-xs font-mono text-zinc-500 max-w-lg">{p.mediaPlaceholder}</p>
