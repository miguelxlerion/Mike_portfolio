...
async function loadData(){
  ...
  try{
    ...
    if(ls){
      ...
      if(d.about) MAIN_ABOUT=d.about;
      else if(d.threeDPage?.about) MAIN_ABOUT=d.threeDPage.about;
      if(d.renders){
...
function applyRendersSettings(remote){
  ...
  renderMarquee();
  // about overlay — profile (MAIN_ABOUT) has priority, renders about is fallback
  const abProfile = (MAIN_ABOUT && Array.isArray(MAIN_ABOUT.paragraphs) && MAIN_ABOUT.paragraphs.length) ? MAIN_ABOUT : null;
  if(abProfile){
    const labEl=document.getElementById('aboutLabel');
    if(labEl) labEl.textContent=abProfile.label||'ABOUT ME';
    const titleEl=document.getElementById('aboutTitle');
    if(titleEl) titleEl.innerHTML=abProfile.title||'';
    const statsEl=document.getElementById('aboutStats');
    if(statsEl) statsEl.style.display='none';
    const paras=abProfile.paragraphs||[];
    const p1=document.getElementById('aboutPara1');
    const p2=document.getElementById('aboutPara2');
    const p3=document.getElementById('aboutPara3');
    if(p1) p1.innerHTML=paras[0]||'';
    if(p2) p2.innerHTML=paras[1]||'';
    if(p3){
      let html=paras[2]||'';
      if(paras[3]) html+='<br><br>'+paras[3];
      // if more than 4 paras, append
      if(paras.length>4) html+='<br><br>'+paras.slice(4).join('<br><br>');
      p3.innerHTML=html;
    }
    const skillsEl=document.getElementById('aboutSkills');
    if(skillsEl){
      if(abProfile.skills && abProfile.skills.length){
        skillsEl.style.display='grid';
        skillsEl.innerHTML=abProfile.skills.map(sk=>`<div class="rounded-xl border border-[#f4efe6]/10 bg-[#0d0c0b]/50 p-4"><div class="mono text-[10px] tracking-[0.2em] text-[#E4572E] mb-3">${(sk.title||'').replace(/</g,'&lt;')}</div><div class="flex flex-wrap gap-2">${(sk.tags||[]).map(t=>`<span class="px-3 py-1 rounded-full bg-[#f4efe6]/5 border border-[#f4efe6]/10 text-[11px] text-[#f4efe6]/70">${String(t).replace(/</g,'&lt;')}</span>`).join('')}</div></div>`).join('');
      } else {
        skillsEl.style.display='none';
        skillsEl.innerHTML='';
      }
    }
    const metaEl=document.getElementById('aboutMeta');
    if(metaEl){
      if(abProfile.subtitle){
        metaEl.innerHTML=`<div class="col-span-3 text-[13px] leading-relaxed text-[#f4efe6]/60">${abProfile.subtitle}</div>`;
    ...
    const skillsEl=document.getElementById('aboutSkills');
    if(skillsEl){ skillsEl.style.display='none'; skillsEl.innerHTML=''; }
    ...
    if(s.aboutMeta){ const metas=document.querySelectorAll('#aboutMeta > div'); s.aboutMeta.forEach((m,i)=>{ if(metas[i]){ metas[i].innerHTML=m.k+'<br><span> '+m.v+'</span>'; } }); const metaEl=document.getElementById('aboutMeta'); if(metaEl) metaEl.style.display='grid'; }
  }
